...
*   **`PA::ActorContext`**：用于游戏中的任何 `Actor` 实体。
    ...
    *   包含 `Actor* actor` 成员。
    *   便捷构造：`ActorContext::from(actor)` 自动填充所有字段。
*   **`PA::MobContext`**：继承自 `ActorContext`，用于 `Mob` 实体。
    *   `kTypeId`：`TypeId("ctx:Mob")`
    *   包含 `Mob* mob` 成员。
    *   便捷构造：`MobContext::from(mob)` 自动填充 `mob` 和 `actor` 字段。
*   **`PA::PlayerContext`**：继承自 `MobContext`，用于 `Player` 实体。
    *   `kTypeId`：`TypeId("ctx:Player")`
    *   包含 `Player* player` 成员。
    *   便捷构造：`PlayerContext::from(player)` 自动填充 `player`、`mob` 和 `actor` 字段。
...
例如，一个玩家可能正在看着一个生物。你希望在玩家的上下文中，获取被看着的生物的生命值。如果已经有一个在 `ActorContext` 下工作的 `{actor_health}` 占位符，你可以注册一个名为 `look` 的别名，它能将 `PlayerContext` 解析为玩家视线所及的 `ActorContext`。这样，你就可以直接使用 `{look:actor_health}` 来获取信息，而无需为“玩家看着的生物的生命值”编写一个全新的占位符。
...
如果调用方不能安全使用 C++ ABI（例如 C、Rust、Zig、Go FFI，或不同编译器/运行时构建的模块），请包含 `PA/PlaceholderCAPI.h`。该头文件只暴露 C ABI 类型：不透明指针、整数、`char*` 和函数指针。

### 生命周期约定

*   `PA_Replace` / `PA_ReplaceServer` 返回的字符串必须使用 `PA_FreeCString` 释放。
*   通过 `PA_CreateOwner` 创建的 owner 用来归属一组 C ABI 注册项。调用 `PA_UnregisterOwner` 会卸载该 owner 下的占位符；调用 `PA_DestroyOwner` 会先卸载再释放 owner。
*   `PA_Create*Context` 返回的上下文必须使用 `PA_DestroyContext` 释放。传入的底层游戏对象指针仍由调用方/服务器持有，C ABI 层不会接管其生命周期。
*   注册占位符时，`PA_CPlaceholderOptions::size` 必须设置为 `sizeof(PA_CPlaceholderOptions)`，用于后续 ABI 扩展兼容。
...
static void eval_hello(
    const PA_CEvaluationContext* ctx,
    ...
    (void)arg_count;
    (void)user_data;
    PA_CStringWriterAppend(out, "hello from C ABI");
}

void register_placeholders(void) {
    PA_COwner* owner = PA_CreateOwner("my_plugin");

    PA_CPlaceholderOptions options = {0};
    options.size = sizeof(options);
    options.owner = owner;
    options.prefix = "c";
    options.token = "hello";
    options.context_type_id = PA_ContextTypeServer();
    options.evaluate = eval_hello;

    PA_RegisterPlaceholder(&options);
}
```
...
*   `PA_WITH_ARGS_P(svc, owner, prefix, ctx_type, token_str, lambda_body)`
*   `PA_SERVER_P(svc, owner, prefix, token_str, lambda_body)`
*   `PA_SERVER_CACHED_P(svc, owner, prefix, token_str, cache_duration, lambda_body)`
*   `PA_SERVER_WITH_ARGS_P(svc, owner, prefix, token_str, lambda_body)`

使用示例：
```cpp
// 注册为 {myplugin:player_level}
PA_SIMPLE_P(svc, owner, "myplugin", PA::PlayerContext, "{player_level}", {
    out = std::to_string(getLevel(c.player));
});
```
