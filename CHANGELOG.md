...
## [0.5.0] 2026-01-27
### Fixed
- 适配bds版本1.21.132，并删除部分默认占位符

## [0.4.8] 2025-11-07
### Fixed
- 修复了 `actor` 占位符在其他插件使用时返回无效值的问题，确保了 `ActorContext`、`MobContext` 和 `PlayerContext` 在别名占位符解析时能够正确创建和初始化。
...
### Fixed
- 修复了 `actor` 占位符在其他插件使用时返回无效值的问题，确保了 `ActorContext`、`MobContext` 和 `PlayerContext` 在别名占位符解析时能够正确创建和初始化。
- 修复了 `player_world_coordinate` 别名占位符中潜在的线程安全问题，现在使用 `std::shared_ptr` 管理 `WorldCoordinateData` 的生命周期。
### Added
- 适配LL1.7.X
